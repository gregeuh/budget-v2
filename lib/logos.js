...
export function devinerDomaine(libelle = "") {
  ...
  // On exige une frontière de mot pour éviter les faux positifs du type
  // "chez paulo" qui contiendrait "paul" (la boulangerie) sans lien réel.
  for (const [cle, domaine] of Object.entries(ANNUAIRE)) {
    if (cle.length < 4) continue; // clés courtes trop permissives en sous-chaîne
    const motif = new RegExp(`(^|[^a-z])${cle.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}([^a-z]|$)`);
