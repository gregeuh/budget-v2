...
// The permanent Powens token is kept in an encrypted, HttpOnly cookie only while the
// connection callback completes. It is bound to the Firebase user that initiated it.
...
export function unsealPowensSession(value) {
  ...
  try {
    ...
    return session?.token && session?.state && session?.uid ? session : null;
  } catch {
    return null;
  }
}
