...
// Préférences d'affichage, définies une fois par le store au chargement.
// euros() les lit sans changer de signature (utilisée à des centaines d'endroits).
let PREF_AFFICHAGE = { centimes: true, arrondiGrandsNombres: true };
export function definirFormatAffichage(pref = {}) {
  PREF_AFFICHAGE = { centimes: true, arrondiGrandsNombres: true, ...pref };
}

export const euros = (n, opts = {}) => {
  const v = n || 0;
  // "precis" force les centimes (fiches, ajustements) : il prime toujours.
  const forcePrecis = opts.precis === true;
  const masqueCentimes = !forcePrecis && PREF_AFFICHAGE.centimes === false;
  const arrondiGrands = PREF_AFFICHAGE.arrondiGrandsNombres !== false;

  let maxDec;
  if (masqueCentimes) maxDec = 0;
  else if (forcePrecis) maxDec = 2;
  else maxDec = Math.abs(v) >= 1000 && arrondiGrands ? 0 : 2;

  const { precis, ...reste } = opts;
...
export const TYPES_COMPTE = {
  ...
  credit: { label: "Crédit / prêt", icone: "🏦", couleur: "corail", groupe: "credits" },
...
export function definirCategoriesPerso(perso = {}) {
  ...
  for (const [cle, c] of Object.entries(perso)) {
    if (!CATEGORIES[cle] && c?.label) {
      toutesCategories[cle] = { label: c.label, icone: c.icone || "🏷️", type: c.type || "envie", couleur: c.couleur || "ciel", perso: true };
    }
  }
}
