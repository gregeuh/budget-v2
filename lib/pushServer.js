...
export async function utilisateursAvecPush() {
  const souscriptions = await firestorePush().collectionGroup("pushSubscriptions").get();
  return [...new Set(souscriptions.docs.map((doc) => doc.ref.parent.parent?.id).filter(Boolean))];
}

export async function chargerDonneesNotification(uid) {
  const firestore = firestorePush();
  const [parametres, transactions, recurrentes] = await Promise.all([
    firestore.doc(`users/${uid}/parametres/app`).get(),
    firestore.collection(`users/${uid}/transactions`).get(),
    firestore.collection(`users/${uid}/recurrentes`).get(),
  ]);
  const app = parametres.data() || {};
  return { profil: app.profil || {}, budgets: app.budgets || {}, transactions: transactions.docs.map((doc) => ({ id: doc.id, ...doc.data() })), recurrentes: recurrentes.docs.map((doc) => ({ id: doc.id, ...doc.data() })) };
}
