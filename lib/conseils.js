import { toutesCategories as CATEGORIES, TYPES_COMPTE, PLAFONDS, euros, cleMois, isoLocal, moisDecaleLocal } from "./format";

const moisDecale = (n) => moisDecaleLocal(n);

export function statsMois(transactions, mois) {
  // Deux formes de virement coexistent : catégorie "virement" (ancienne) et
  // champ versId (moderne). On écarte les deux : déplacer son argent n'est
  // ni une dépense ni un revenu.
  const txs = transactions.filter(
    (t) => cleMois(t.date) === mois && !t.versId && (CATEGORIES[t.categorie] || CATEGORIES.autre).type !== "virement"
  );
...
// Quand la paie tombe en fin de mois, elle finance le mois suivant. Cette vue
// analytique conserve les dates comptables, mais rattache les opérations depuis
// le jour de paie au mois budgétaire suivant (ex. paie du 31 juillet → août).
export function statsMoisBudgetaire(transactions, mois, jourSalaire = 0) {
  const jour = Number(jourSalaire) || 0;
  if (jour < 20) return statsMois(transactions, mois);
  const rattache = (date) => {
    if (!date || Number(date.slice(8, 10)) < jour) return date;
    const [annee, numero] = date.slice(0, 7).split("-").map(Number);
    const suivant = new Date(annee, numero, 1);
    return `${suivant.getFullYear()}-${String(suivant.getMonth() + 1).padStart(2, "0")}${date.slice(7)}`;
  };
  return statsMois(transactions.map((t) => ({ ...t, date: rattache(t.date) })), mois);
}
...
export function genererConseils({ comptes, transactions, soldes, budgets, profil, credits = [], projets = [] }) {
  ...
  // Même référence que le score santé : on évite de rapporter le mois à un
  // petit encaissement isolé alors qu'un revenu mensuel est configuré.
  const revenu = Math.max(0, s.revenus || 0, profil.revenuMensuel || 0);
  ...
  // 4bis. Compte à découvert
  for (const c of comptes) {
    const groupe = (TYPES_COMPTE[c.type] || TYPES_COMPTE.autre).groupe;
    const solde = soldes[c.id] || 0;
    if ((groupe === "courant" || groupe === "avantages") && solde < 0) {
      conseils.push({ ton: "alerte", icone: "⚠️", titre: `${c.nom} à découvert`, texte: `Solde actuel : ${euros(solde)}. Les agios de découvert coûtent cher — si c'est un oubli de saisie, pense au réajustement de solde depuis la fiche du compte.` });
    }
  }
  ...
  if (depensesMoyennes > 0) {
    const moisCouverts = Math.max(0, epargneDispo) / depensesMoyennes;
    if (moisCouverts < 3) {
  ...
  // Clé stable par conseil, pour pouvoir le masquer durablement
  for (const c of conseils) {
    if (!c.cle) c.cle = c.titre.toLowerCase().replace(/[0-9%€.,\s]+/g, "-").replace(/^-|-$/g, "");
  }
  return conseils.sort((a, b) => ordre[a.ton] - ordre[b.ton]);
}

// Résumé complet envoyé au coach IA (via la clé API du propriétaire de l'app).
// Contient : soldes nommés, 6 mois d'historique par catégorie, opérations récentes
// avec libellés, récurrences, projets, crédits, budgets, profil budgétaire et score.
export function resumePourCoach({ comptes, transactions, soldes, budgets, profil, credits = [], projets = [], recurrentes = [], categories = {} }) {
  const moisCle = (n) => moisDecaleLocal(n);

  // 6 derniers mois agrégés
  const historique = {};
  for (let i = 0; i >= -5; i--) {
    const m = moisCle(i);
    const st = statsMois(transactions, m);
    if (st.revenus === 0 && st.depenses === 0) continue;
    historique[m] = {
      revenus: Math.round(st.revenus),
      depenses: Math.round(st.depenses),
      parCategorie: Object.fromEntries(Object.entries(st.parCategorie).map(([k, v]) => [k, Math.round(v)])),
    };
  }

  // Opérations des 60 derniers jours (plafonnées à 150, libellés tronqués)
  const depuis = isoLocal(new Date(Date.now() - 60 * 86400000));
  const nomCompte = (id) => comptes.find((c) => c.id === id)?.nom || "?";
  const recentes = transactions
    .filter((t) => t.date >= depuis)
    ...
    .map((t) => ({
      ...
      libelle: (t.libelle || "").slice(0, 40),
      compte: nomCompte(t.compteId),
      ...(t.versId ? { virementVers: nomCompte(t.versId) } : {}),
      ...(t.horsSolde ? { horsSolde: true } : {}),
    }));

  // Libellés des catégories (y compris personnalisées) pour que les clés soient lisibles
  const dicoCategories = Object.fromEntries(
    Object.entries(categories).map(([cle, c]) => [cle, `${c.label} (${c.type})`])
  );
  ...
  return {
    ...
    profil: {
      ...
      jourDuSalaire: profil.jourSalaire || null,
      regleSalaire: profil.modeSalaire || "jour",
    },
    comptes: comptes.map((c) => ({ nom: c.nom, type: c.type, solde: Math.round(soldes[c.id] || 0) })),
    historiqueParMois: historique,
    operationsRecentes: recentes,
    recurrentes: recurrentes
      .filter((r) => r.actif !== false)
      .map((r) => ({ libelle: r.libelle, montant: r.montant, frequence: r.frequence, prochaine: r.prochaine })),
    budgets,
    credits: credits.map((c) => ({ nom: c.nom, restant: Math.round(c.restant || 0), mensualite: Math.round(c.mensualite || 0), taux: c.taux || 0 })),
    projets: projets.map((p) => ({ nom: p.nom, objectif: Math.round(p.objectif || 0), epargne: Math.round(p.montantActuel || 0), echeance: p.echeance || null })),
    abonnementsDetectes: detecterAbonnements(transactions).map((a) => ({ libelle: a.libelle, montant: a.montant })),
    categoriesDictionnaire: dicoCategories,
  };
}
