...
export function calculerProjection({ comptes, soldes, transactions, recurrentes, profil }) {
  ...
  const salaireISO = prochaineDateSalaire(profil.jourSalaire, profil.modeSalaire);
  ...
  let prevu = 0;
  let attendu = 0;
  let soldeProjete = dispo;
  const evolution = [{ date: auj, solde: dispo }];
  for (const t of aVenir) {
    ...
    if (impact !== 0) {
      soldeProjete += impact;
      evolution.push({ date: t.date, solde: soldeProjete });
    }
  }
  ...
  return { dispo, prevu, attendu, reste, jours, salaireISO, horizonISO, aVenir, evolution, parJour: reste / jours };
}
