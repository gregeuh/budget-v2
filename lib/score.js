import { TYPES_COMPTE, aujourdhui, cleMois, isoLocal, salaireConfigure } from "./format";
...
export function calculerScore({ transactions, comptes, soldes, budgets, credits, recurrentes, profil }) {
  ...
  // Un petit remboursement ou une vente ponctuelle ne doit pas remplacer le
  // revenu mensuel déclaré comme base des ratios (épargne, endettement).
  ...
  const aJourSalaire = salaireConfigure(profil);
  piliers.push({
    id: "regularite",
    icone: "📆",
    label: "Régularité",
  ...
  const niveau =
    total >= 80 ? { label: "Excellent", emoji: "🏆", couleur: "var(--menthe)" } :
    total >= 60 ? { label: "Solide", emoji: "💪", couleur: "var(--marque)" } :
    total >= 40 ? { label: "En construction", emoji: "🧱", couleur: "var(--beurre)" } :
    { label: "Fragile", emoji: "🌱", couleur: "var(--corail)" };
