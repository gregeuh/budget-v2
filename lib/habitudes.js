 ...
 * Pour chaque commerçant : la catégorie, le lieu et l'icône les plus fréquents.
 * Retourne une Map : cleCommercant -> { categorie, lieu, icone, libelle, occurrences }
 */
export function construireMemoire(transactions = []) {
  ...
  for (const t of transactions) {
    ...
    if (!memoire.has(k)) {
      memoire.set(k, { libelle: source.trim(), cats: {}, lieux: {}, icones: {}, occurrences: 0, derniere: t.date || "" });
    }
    ...
    if (t.icone && String(t.icone).trim()) {
      const icone = String(t.icone).trim();
      e.icones[icone] = (e.icones[icone] || 0) + 1;
    }
