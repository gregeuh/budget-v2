...
import ConnexionBancaire from "@/components/ConnexionBancaire";
...
export default function ReglagesContenu() {
  ...
  return (
    <div className="flex min-h-full flex-col">
      ...
      <section className="mb-5"><h2 className="mb-2 px-1 text-v3-caption font-semibold uppercase tracking-wide text-ui-text-secondary">Organisation</h2><nav className="overflow-hidden rounded-v3-m bg-ui-surface-floating shadow-v3-soft"><a href="/inbox" className="flex items-center gap-3 px-4 py-3.5 active:bg-ui-surface-raised"><span className="text-xl">🧭</span><span className="min-w-0 flex-1"><span className="block text-sm font-semibold">Boîte financière</span><span className="block truncate text-xs text-sourdine">Mes priorités et anomalies à traiter</span></span><span className="text-sourdine">›</span></a><Rangee icone="🏦" label="Connecter ma banque" detail="Synchroniser comptes et opérations" onClick={() => setFiche("banque")} /><Rangee icone="🔁" label="Récurrentes" detail={nbRecurrentes > 0 ? `${nbRecurrentes} opérations actives` : "Anticiper les prochaines échéances"} onClick={() => setFiche("recurrentes")} /><Rangee icone="✨" label="Règles automatiques" detail={(profil.reglesAuto || []).length ? `${profil.reglesAuto.length} règle${profil.reglesAuto.length > 1 ? "s" : ""} active${profil.reglesAuto.length > 1 ? "s" : ""}` : "Classer automatiquement tes commerçants"} onClick={() => setFiche("regles")} /><Rangee icone="💼" label="Salaire & charges fixes" detail="Projection et reste à vivre" onClick={() => setFiche("assistant")} /><Rangee icone="📥" label="Importer un relevé bancaire" detail="Ajouter un fichier CSV" onClick={() => setFiche("import")} /><Rangee icone="📺" label="Centre d’abonnements" detail="Coût mensuel, doublons et économies" onClick={() => setFiche("abonnements")} /><Rangee icone="✨" label="Nettoyer les libellés" detail="Uniformiser les intitulés" onClick={() => setFiche("renommer")} /><Rangee icone="🏷️" label="Ranger mes opérations" detail="Catégoriser les transactions" onClick={() => setFiche("categoriser")} dernier /></nav></section>

      <section><h2 className="mb-2 px-1 text-v3-caption font-semibold uppercase tracking-wide text-ui-text-secondary">Données</h2><nav className="overflow-hidden rounded-v3-m bg-ui-surface-floating shadow-v3-soft"><Rangee icone="🔔" label="Notifications" detail="Alertes sur cet iPhone" onClick={() => setFiche("notifications")} /><Rangee icone="🔐" label="Confidentialité & iPhone" detail="Installation, hors-ligne et bonnes pratiques" onClick={() => setFiche("confidentialite")} /><Rangee icone="🧹" label="Qualité des données" detail={pointsQualite ? `${pointsQualite} point${pointsQualite > 1 ? "s" : ""} à vérifier` : "Tout est bien rangé"} onClick={() => setFiche("qualite")} /><Rangee icone="💾" label="Sauvegarde & données" detail="Exporter ou restaurer mes informations" onClick={() => setFiche("donnees")} /><Rangee icone="🩺" label="Journal technique" detail="Diagnostic de l’application" onClick={() => setFiche("journal")} dernier /></nav></section>
      ...
      {fiche === "banque" && <ConnexionBancaire onFermer={() => setFiche(null)} />}
      ...
      {fiche === "notifications" && <NotificationsPush onFermer={() => setFiche(null)} />}
    </div>
  );
}
