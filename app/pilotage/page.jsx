...
const PREPARER = [
  { href: "/plan", icone: "☷", titre: "Plan mensuel", detail: "Répartis revenus, charges et épargne", ton: "bg-marque-pale" },
  { href: "/abonnements", icone: "↻", titre: "Abonnements", detail: "Retrouve leur coût et les doublons", ton: "bg-beurre-pale" },
  { href: "/regles", icone: "⚡", titre: "Règles auto", detail: "Classe tes prochaines opérations", ton: "bg-menthe-pale" },
];
...
export default function PilotagePage() {
  ...
  return <div className="space-y-5">
    ...
    <section><div className="mb-2 flex items-baseline justify-between"><h2 className="font-semibold">Préparer & automatiser</h2><span className="text-xs text-sourdine">moins de saisie</span></div><div className="space-y-2">{PREPARER.map((outil) => <Link key={outil.href} href={outil.href} className={`flex items-center gap-3 rounded-v3-m p-3.5 shadow-v3-soft transition-transform active:scale-[0.99] ${outil.ton}`}><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-ui-surface-floating text-xl">{outil.icone}</span><span className="min-w-0 flex-1"><strong className="block text-sm">{outil.titre}</strong><span className="block text-xs text-sourdine">{outil.detail}</span></span><span className="text-lg text-sourdine">›</span></Link>)}</div></section>
  </div>;
}
