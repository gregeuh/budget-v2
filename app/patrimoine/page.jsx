...
export default function PatrimoinePage() {
  ...
  return <div className="space-y-5">
    <header><p className="text-v3-caption font-semibold uppercase tracking-[.14em] text-marque">Tes espaces d’argent</p><h1 className="mt-1 text-3xl font-bold tracking-tight">Où est ton argent ?</h1><p className="mt-2 text-sm leading-5 text-sourdine">Comptes du quotidien, épargne et crédits : tout reste lisible au même endroit.</p></header>
    <section className="overflow-hidden rounded-v3-l bg-[linear-gradient(145deg,var(--marque),var(--marque-texte))] p-5 text-white shadow-v3-medium"><p className="text-sm text-white/70">Total de tes espaces</p><p className="mt-1 tnum text-4xl font-bold">{euros(total)}</p><div className="mt-5 grid grid-cols-2 gap-3 border-t border-white/15 pt-4 text-sm"><span><span className="block text-white/60">Épargne</span><strong className="mt-1 block">{euros(epargne)}</strong></span><span><span className="block text-white/60">Crédits</span><strong className="mt-1 block">{credits.length} compte{credits.length > 1 ? "s" : ""}</strong></span></div></section>
